...
import { updateIcons, normalizeIngredientName, showFeedback, showConfirm } from '../utils.js';

export function updateIngredientSuggestions() {
    const allIngredients = [...state.recipes, ...state.exploreRecipes].flatMap(r => r.ingredients || []);
...
export function renderCategories() {
    ...
    container.innerHTML = CATEGORIES.filter(cat => state.currentView === 'home' || cat !== 'Favorites').map(cat => `
        <button 
...
export function renderRecipes() {
    ...
    let source = state.currentView === 'explore' ? state.exploreRecipes : state.recipes;
    ...
    } else if (state.currentCategory !== 'All' && state.currentCategory !== 'Favorites') {
    ...
    if (state.searchQuery) {
        filtered = filtered.filter(r => 
            ...
            (r.ingredients && r.ingredients.some(i => i.toLowerCase().includes(state.searchQuery)))
        );
    }
            ...
            <div onclick="${isExplore ? `openExploreDetail(${recipeIdOrIndex})` : `openDetail(${recipe.id})`}" class="m3-card group cursor-pointer flex flex-col h-full rounded-[28px] md:rounded-[32px] overflow-hidden">
                ...
                <div class="px-2 pb-2">
                    <div class="flex items-start justify-between gap-2 mb-2">
                        ...
                        ${recipe.isFavorite ? '<i data-lucide="heart" class="w-4 h-4 fill-[#B3261E] text-[#B3261E] mt-1 flex-shrink-0"></i>' : ''}
                    </div>
...
export async function addToHome(e, exploreIndex) {
    if (e) e.stopPropagation();
    ...
    try {
        ...
        renderRecipes();
        showFeedback('Recipe Added!', `${recipe.title} has been added to your collection.`);
